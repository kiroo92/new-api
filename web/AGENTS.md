...
- **公开定价**：`/pricing` 默认为定价总览，`?section=models` 为原模型广场；旧的带筛选参数链接仍进入模型广场。总览按接口 `routing_groups` 的路由顺序，为每个模型展示首个可用分组的费率并标注分组，不取其他分组最低价；访客使用新账户 `default` 的分组权限，已删除的渠道分组不参与报价。`features/pricing/lib/overview-prices.ts` 的官方参考价仅用于展示，维护时核实来源和日期，不影响实际计费；未知型号不推断官方价，条件计费不宣称固定节省比例。两种视图共用价格接口、详情和表格组件。
