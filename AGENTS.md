...
- Each user has one model API key (`Token.IsDefault`), created in the registration transaction (password, OAuth, WeChat and admin creation). Startup migration retains the oldest non-deleted Key/ID/usage, soft-deletes extra credentials and seeds accounts without a Key. Key-level model/expiry/quota restrictions are removed; account group access, balance checks and account suspension still apply. `POST /api/token/regenerate` requires a one-time `api_key.regenerate` security proof. Rotation preserves token ID and accounting; old credentials fail database-backed lookup even with stale Redis data. Personal access tokens and login sessions are separate.
- Default API keys resolve all permitted, non-deleted channel groups through `service.GetUserRoutingGroups`: existing account group first, then configured Auto order, then remaining groups sorted by name. Model discovery and relay share this resolution; the legacy per-token Auto cap does not apply. Pricing exposes the same order as `routing_groups`. Deleting a channel group does not require rewriting the account's membership group.
- EPay top-up and subscription payment are already implemented with `github.com/Calcium-Ion/go-epay`. For an EPUSDT deployment, set `PayAddress` to the EPUSDT EPay base path without the final `/submit.php` (`.../payments/epay/v1/order/create-transaction`); the client appends that suffix. Use the EPUSDT API-key PID as `EpayId` and its secret as `EpayKey`. Configure crypto methods with EPUSDT selectors such as `usdt.tron` (not `usdt-tron`); keep `PayMethods` free of unsupported `wxpay`/`alipay` entries for a crypto-only gateway. NewAPI receives top-up callbacks at `/api/user/epay/notify` and subscription callbacks at `/api/subscription/epay/notify`; both GET and POST are routed. Online EPay is gated by payment-compliance confirmation, non-empty credentials, and at least one configured method.
